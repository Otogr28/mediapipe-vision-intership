...
> **UPDATE (round 10 — DONE + DEPLOYED 2026-07-09):** the GPU body-pose backend
> (lever #3 below, "the real fix") is live on the Jetson. `HALL_POSE_INFERENCE=gpu`
> runs BlazePose person-det + pose-landmark on onnxruntime/TensorRT
> (`detection/gpu_pose.py`). Measured on-device: **pose_fps ~13 -> ~30**, matching
> the hands' rate — **the body-vs-hand speed mismatch is gone.** 4 TRT engines
> cached, ~2.6 GB free, kiosk healthy. See SHARED.md round 10.
>
> Ops notes for next time: models under `models/gpu/` are gitignored so the git
> auto-update does NOT carry them — `rsync models/gpu/` separately (done). The
> Jetson now key-auths without the browser check via **`ssh yahboom`** (Tailscale
> port 2222). `HALL_TRT_MAX_WORKSPACE=512` (hallrun) bounds a cold rebuild on the 8
> GB board. If pose STILL feels behind live (the camera is 30 fps too), levers
> #2/#4/#5 below remain; but the ~13->30 jump should settle the main complaint.
