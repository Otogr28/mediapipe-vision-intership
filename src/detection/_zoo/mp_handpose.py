...
# CUDA execution provider. All pre/post-processing math (crop/rotate, landmark
# de-rotation, bbox) is the upstream implementation unchanged. cv2 is still
# used for image ops; only the neural-net forward pass moved to onnxruntime.
import numpy as np
import cv2 as cv
...
class MPHandPose:
    def __init__(self, modelPath, confThreshold=0.8, backendId=0, targetId=0, providers=None):
        ...
        # Deterministic output order: sorted names give
        # ["Identity", "Identity_1", "Identity_2", "Identity_3"], which is the
        # order _postprocess unpacks (landmarks, conf, handedness,
        # landmarks_word). Verified against the ONNX file:
        #   Identity   [1,63] screen landmarks (x,y,z)
        #   Identity_1 [1,1]  confidence
        #   Identity_2 [1,1]  handedness (0=left .. 1=right)
    ...
    def infer(self, image, palm):
        ...
        # Forward (contiguous float32 NHWC for onnxruntime; output_blob is a
        # list of numpy arrays in self.output_names order so _postprocess is
        # unchanged).
