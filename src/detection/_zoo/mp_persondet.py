...
# Only the inference backend is changed: cv.dnn.readNet/forward is replaced with
# an onnxruntime InferenceSession so the BlazePose person detector can run on the
# Jetson's CUDA/TensorRT execution provider (CPU fallback elsewhere). All
# pre/post-processing math (SSD anchors, NMS, box + 4-keypoint decode) is the
# upstream implementation unchanged. cv2 is still used for image ops
...
class MPPersonDet:
    def __init__(self, modelPath, nmsThreshold=0.3, scoreThreshold=0.5, topK=5000,
        ...
        # ordering: sorted names give ["Identity:0", "Identity_1:0"], which is the
        # order _postprocess expects: output_blob[0] = box+keypoint deltas
        # [1,2254,12] ("Identity:0"), output_blob[1] = scores [1,2254,1]
        # ("Identity_1:0"). Verified against the ONNX file.
    ...
    def infer(self, image):
        ...
        # Forward (onnxruntime needs a contiguous float32 NCHW tensor; the
        # preprocess already yields NCHW float, this just guarantees dtype +
